...
subroutine ca_estdt_burning(lo, hi, sold, so_lo, so_hi, &
 ...
 enddo
  ...
  subroutine ca_estdt_burning_single_zone(i, j, k, &
       sold, so_lo, so_hi, &
       snew, sn_lo, sn_hi, &
       rold, ro_lo, ro_hi, &
       rnew, rn_lo, rn_hi, &
       dx, dt) &
       bind(C, name="ca_estdt_burning_single_zone")
    ! Reactions-limited timestep
    !
    ! .. note::
    !    Binds to C function ``ca_estdt_burning``
    ...
    integer,  intent(in) :: so_lo(3), so_hi(3)
    ! type(c_ptr) :: ca_estdt_burning_single_zone
    integer,  intent(in) :: sn_lo(3), sn_hi(3)
    integer,  intent(in) :: ro_lo(3), ro_hi(3)
    integer,  intent(in) :: rn_lo(3), rn_hi(3)
    ...
    !    do j = lo(2), hi(2)
        ...
        !   do i = lo(1), hi(1)
             ...
             if (.not. ok_to_burn(state_new)) return
             ...
             dt = min(dtnuc_e * e / dedt, dtnuc_X * minval(X / dXdt))
    ...
    end subroutine ca_estdt_burning_single_zone
#endif
