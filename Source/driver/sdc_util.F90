...
contains
  ...
  subroutine f_sdc_jac(n, U, f, Jac, ldjac, iflag, npar, rpar)
    ...
    use rpar_sdc_module
    use meth_params_module, only : nvar, URHO, UFS, UEINT, UEDEN, UMX, UMZ, UTEMP, sdc_solve_for_rhoe, sdc_use_analytic_jac
    use network, only : nspec, nspec_evolve
    use burn_type_module
    use react_util_module
    ...
    if (sdc_use_analytic_jac == 1) then
       ...
       do ncol = 0, n-1
          ...
          call f_sdc(n, U_pert, f_pert, iflag, npar, rpar)
    ...
    do k = lo(3), hi(3)
       do j = lo(2), hi(2)
          do i = lo(1), hi(1)
             ...
             ! if we still failed, then we hit the limit of subcycles
             if (failed) then
