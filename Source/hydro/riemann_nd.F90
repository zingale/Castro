...
contains
  ...
  subroutine riemanncg(i, j, k, &
    ...
    if (idir == 1) then
       ...
       if (special_bnd_lo .and. i == domlo(1) .or. &
           special_bnd_hi .and. i == domhi(1)+1) bnd_fac = ZERO
       ...
       if (special_bnd_lo .and. j == domlo(2) .or. &
           special_bnd_hi .and. j == domhi(2)+1) bnd_fac = ZERO
       ...
       if (special_bnd_lo .and. k == domlo(3) .or. &
           special_bnd_hi .and. k == domhi(3)+1) bnd_fac = ZERO
    ...
    qint(QGAME) = frac*gamstar + (ONE-frac)*gameo
    ...
    qint(iu) = min(abs(qint(iu)), riemann_speed_limit) * sign(ONE, qint(iu))

    ! compute the total energy from the internal, p/(gamma - 1), and the kinetic
    qint(QREINT) = qint(QPRES)/(qint(QGAME) - ONE)
    ...
    do ipassive = 1, npassive
       ...
       if (ustar > ZERO) then
          qint(nqp) = ql(i,j,k,nqp,comp)
       else if (ustar < ZERO) then
          qint(nqp) = qr(i,j,k,nqp,comp)
       else
  ...
  subroutine riemannus(i, j, k, &
    ...
    qint(QGAME) = pgdnv_g/regdnv_g + ONE
...
#else
    qint(QGAME) = qint(i,j,k,QPRES)/regdnv + ONE
    qint(QPRES) = max(qint(i,j,k,QPRES),small_pres)
    qint(QREINT) = regdnv
#endif
    ...
    if (use_eos_in_riemann == 1) then
       ...
       qint(QGAME) = eos_state % p / (eos_state % rho * eos_state % e) + ONE
