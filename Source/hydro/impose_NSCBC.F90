module gc_nscbc_mod
  ...
  use amrex_constants_module, only: ONE, TWO, HALF, ZERO
...
contains
  ...
  subroutine impose_NSCBC(lo, hi, domlo, domhi, &
       ...
       time,delta,dt,verbose) bind(C, name="ca_impose_NSCBC")

    use network, only : nspec
    use fundamental_constants_module, only: k_B, n_A
    use prob_params_module, only : physbc_lo, physbc_hi, problo, probhi, UserBC

    use meth_params_module, only : NVAR, NQAUX, NQ
    ...
    integer, value, intent(in) :: flag_nscbc_isAnyPerio
    ...
    double precision, intent(inout) :: uin(uin_lo(1):uin_hi(1),uin_lo(2):uin_hi(2),uin_lo(3):uin_hi(3),NVAR)
    ...
    double precision, value, intent(in) :: dt, time
    ...
    x_bcMask(x_bcMask_lo(1):x_bcMask_hi(1),x_bcMask_lo(2):x_bcMask_hi(2),x_bcMask_lo(3):x_bcMask_hi(3)) = 0
    y_bcMask(y_bcMask_lo(1):y_bcMask_hi(1),y_bcMask_lo(2):y_bcMask_hi(2),y_bcMask_lo(3):y_bcMask_hi(3)) = 0
    z_bcMask(z_bcMask_lo(1):z_bcMask_hi(1),z_bcMask_lo(2):z_bcMask_hi(2),z_bcMask_lo(3):z_bcMask_hi(3)) = 0
  ...
  subroutine normal_derivative(i, j, k, idir, isign, delta, &
    ...
    use meth_params_module, only : NQ, QPRES, QU, QV, QW, QRHO
    ...
    if (idir == 1) then
       if (isign == 1) then
          ...
          ! 2nd order
          dp = ((-3.0e0_rt/TWO)*q(i,j,k,QPRES)+TWO*q(i+1,j,k,QPRES)-HALF*q(i+2,j,k,QPRES))/delta
          du = ((-3.0e0_rt/TWO)*q(i,j,k,QU)+TWO*q(i+1,j,k,QU)-HALF*q(i+2,j,k,QU))/delta
          dv = ((-3.0e0_rt/TWO)*q(i,j,k,QV)+TWO*q(i+1,j,k,QV)-HALF*q(i+2,j,k,QV))/delta
          dw = ((-3.0e0_rt/TWO)*q(i,j,k,QW)+TWO*q(i+1,j,k,QW)-HALF*q(i+2,j,k,QW))/delta
          drho = ((-3.0e0_rt/TWO)*q(i,j,k,QRHO)+TWO*q(i+1,j,k,QRHO)-HALF*q(i+2,j,k,QRHO))/delta
       ...
       elseif (isign == -1) then
          ...
          call amrex_abort("Problem of isign in impose_NSCBC_3d:normal_derivative")
       end if
    ...
    elseif (idir == 2) then
       ...
       if (isign == 1) then
          ...
          !2nd order
          dp = ((-3.0e0_rt/TWO)*q(i,j,k,QPRES)+TWO*q(i,j+1,k,QPRES)-HALF*q(i,j+2,k,QPRES))/delta
          du = ((-3.0e0_rt/TWO)*q(i,j,k,QU)+TWO*q(i,j+1,k,QU)-HALF*q(i,j+2,k,QU))/delta
          dv = ((-3.0e0_rt/TWO)*q(i,j,k,QV)+TWO*q(i,j+1,k,QV)-HALF*q(i,j+2,k,QV))/delta
          dw = ((-3.0e0_rt/TWO)*q(i,j,k,QW)+TWO*q(i,j+1,k,QW)-HALF*q(i,j+2,k,QW))/delta
          drho = ((-3.0e0_rt/TWO)*q(i,j,k,QRHO)+TWO*q(i,j+1,k,QRHO)-HALF*q(i,j+2,k,QRHO))/delta
    ...
    elseif (idir == 3) then
       ...
       if (isign == 1) then
          ...
          !2nd order
          dp = ((-3.0e0_rt/TWO)*q(i,j,k,QPRES)+TWO*q(i,j,k+1,QPRES)-HALF*q(i,j,k+2,QPRES))/delta
          du = ((-3.0e0_rt/TWO)*q(i,j,k,QU)+TWO*q(i,j,k+1,QU)-HALF*q(i,j,k+2,QU))/delta
          dv = ((-3.0e0_rt/TWO)*q(i,j,k,QV)+TWO*q(i,j,k+1,QV)-HALF*q(i,j,k+2,QV))/delta
          dw = ((-3.0e0_rt/TWO)*q(i,j,k,QW)+TWO*q(i,j,k+1,QW)-HALF*q(i,j,k+2,QW))/delta
          drho = ((-3.0e0_rt/TWO)*q(i,j,k,QRHO)+TWO*q(i,j,k+1,QRHO)-HALF*q(i,j,k+2,QRHO))/delta
       ...
       call amrex_abort("Problem of idir in impose_NSCBC_3d:normal_derivative")
    end if
  ...
  subroutine tangential_derivative(i, j, k, idir, delta, &
    ...
    if (idir == 1) then
       ...
       ! 2nd order Central
       dp = (q(i+1,j,k,QPRES)-q(i-1,j,k,QPRES))/(TWO*delta)
       du = (q(i+1,j,k,QU)-q(i-1,j,k,QU))/(TWO*delta)
       dv = (q(i+1,j,k,QV)-q(i-1,j,k,QV))/(TWO*delta)
       dw = (q(i+1,j,k,QW)-q(i-1,j,k,QW))/(TWO*delta)
       drho = (q(i+1,j,k,QRHO)-q(i-1,j,k,QRHO))/(TWO*delta)

    elseif (idir == 2) then

       ! 2nd order Central
       dp = (q(i,j+1,k,QPRES)-q(i,j-1,k,QPRES))/(TWO*delta)
       du = (q(i,j+1,k,QU)-q(i,j-1,k,QU))/(TWO*delta)
       dv = (q(i,j+1,k,QV)-q(i,j-1,k,QV))/(TWO*delta)
       dw = (q(i,j+1,k,QW)-q(i,j-1,k,QW))/(TWO*delta)
       drho = (q(i,j+1,k,QRHO)-q(i,j-1,k,QRHO))/(TWO*delta)

    elseif (idir == 3) then

       ! 2nd order Central
       dp = (q(i,j,k+1,QPRES)-q(i,j,k-1,QPRES))/(TWO*delta)
       du = (q(i,j,k+1,QU)-q(i,j,k-1,QU))/(TWO*delta)
       dv = (q(i,j,k+1,QV)-q(i,j,k-1,QV))/(TWO*delta)
       dw = (q(i,j,k+1,QW)-q(i,j,k-1,QW))/(TWO*delta)
       drho = (q(i,j,k+1,QRHO)-q(i,j,k-1,QRHO))/(TWO*delta)

    else
       call amrex_abort("Problem of idir in impose_NSCBC_3d:tangential_derivative")
    end if
  ...
  subroutine compute_transverse_terms(i, j, k, idir,  &
    ...
    use meth_params_module, only : NQ, QPRES, QU, QV, QW, QRHO, NQAUX, QC, QGAMC
    ...
    elseif (idir == 3) then
       ...
       call amrex_abort("Problem of idir in impose_NSCBC_2d:compute_transverse_terms")
    end if
  ...
  subroutine update_ghost_cells(i, j, k, bc_type, idir, isign, delta, &
    ...
    use meth_params_module, only : NVAR, URHO, UMX, UMY, UMZ, UEDEN, UEINT, UTEMP,&
         UFS, NQAUX, QC, QGAMC, &
         QDPDE, QDPDR, &
         NQ, QRHO, QU, QV, QW, QREINT, QPRES, QTEMP, &
         QFS, QFX, QGAME, NHYP
    ...
    if ((idir == 1) .or. (idir == 2) .or. (idir == 3)) then
       ...
       call amrex_abort("Problem of idir in impose_NSCBC_3d:update_ghost_cells")
    end if

    if ((isign == 1) .or. (isign == -1)) then
       continue
    else
       call amrex_abort("Problem of isign in impose_NSCBC_3d:update_ghost_cells")
    end if
    ...
    if (idir == 1) then
       local_index = i
       drho = (L2 + HALF*(L1 + L5))/(qaux(i,j,k,QC)**TWO)
       du   = (L5-L1)/(TWO*qaux(i,j,k,QC)*q(i,j,k,QRHO))
       dv   = L3
       dw   = L4
       dp   = HALF*(L1+L5)
    elseif (idir == 2) then
       local_index = j
       drho = (L3 + HALF*(L1 + L5))/(qaux(i,j,k,QC)**TWO)
       du   = L2
       dv   = (L5-L1)/(TWO*qaux(i,j,k,QC)*q(i,j,k,QRHO))
       dw   = L4
       dp   = HALF*(L1+L5)
    elseif (idir == 3) then
       local_index = k
       drho = (L4 + HALF*(L1 + L5))/(qaux(i,j,k,QC)**TWO)
       du   = L2
       dv   = L3
       dw   = (L5-L1)/(TWO*qaux(i,j,k,QC)*q(i,j,k,QRHO))
       dp   = HALF*(L1+L5)
    endif
    ...
    if (idir == 1) then
       ...
       ! Update ghost cells
       ! 2nd order
       q(idx_gc1,j,k,QU)    = q(idx_int1,j,k,QU) - TWO*delta*du*isign
       q(idx_gc1,j,k,QV)    = q(idx_int1,j,k,QV) - TWO*delta*dv*isign
       q(idx_gc1,j,k,QW)    = q(idx_int1,j,k,QW) - TWO*delta*dw*isign
       q(idx_gc1,j,k,QRHO)  = q(idx_int1,j,k,QRHO)  - TWO*delta*drho*isign
       q(idx_gc1,j,k,QPRES) = q(idx_int1,j,k,QPRES) - TWO*delta*dp*isign

       !----------------
       q(idx_gc2,j,k,QU)    = -TWO*q(idx_int1,j,k,QU) - 3.0e0_rt*q(i,j,k,QU) + 6.0e0_rt*q(idx_gc1,j,k,QU) + 6.0e0_rt*delta*du*isign
       q(idx_gc2,j,k,QV)    = -TWO*q(idx_int1,j,k,QV) - 3.0e0_rt*q(i,j,k,QV) + 6.0e0_rt*q(idx_gc1,j,k,QV) + 6.0e0_rt*delta*dv*isign
       q(idx_gc2,j,k,QW)    = -TWO*q(idx_int1,j,k,QW) - 3.0e0_rt*q(i,j,k,QW) + 6.0e0_rt*q(idx_gc1,j,k,QW) + 6.0e0_rt*delta*dw*isign
       q(idx_gc2,j,k,QRHO)  = -TWO*q(idx_int1,j,k,QRHO) - 3.0e0_rt*q(i,j,k,QRHO) + 6.0e0_rt*q(idx_gc1,j,k,QRHO) + 6.0e0_rt*delta*drho*isign
       q(idx_gc2,j,k,QPRES) = -TWO*q(idx_int1,j,k,QPRES) - 3.0e0_rt*q(i,j,k,QPRES) + 6.0e0_rt*q(idx_gc1,j,k,QPRES) + 6.0e0_rt*delta*dp*isign

       q(idx_gc3,j,k,QU)    = 3.0e0_rt*q(idx_int1,j,k,QU) +10.0e0_rt*q(i,j,k,QU) - 18.0e0_rt*q(idx_gc1,j,k,QU) &
            + 6.0e0_rt*q(idx_gc2,j,k,QU) - 12.0e0_rt*delta*du*isign
       q(idx_gc3,j,k,QV)    = 3.0e0_rt*q(idx_int1,j,k,QV) +10.0e0_rt*q(i,j,k,QV) - 18.0e0_rt*q(idx_gc1,j,k,QV) &
            + 6.0e0_rt*q(idx_gc2,j,k,QV) - 12.0e0_rt*delta*dv*isign
       q(idx_gc3,j,k,QW)    = 3.0e0_rt*q(idx_int1,j,k,QW) +10.0e0_rt*q(i,j,k,QW) - 18.0e0_rt*q(idx_gc1,j,k,QW) &
            + 6.0e0_rt*q(idx_gc2,j,k,QW) - 12.0e0_rt*delta*dw*isign
       q(idx_gc3,j,k,QRHO)    = 3.0e0_rt*q(idx_int1,j,k,QRHO) +10.0e0_rt*q(i,j,k,QRHO) - 18.0e0_rt*q(idx_gc1,j,k,QRHO) &
            + 6.0e0_rt*q(idx_gc2,j,k,QRHO) - 12.0e0_rt*delta*drho*isign
       q(idx_gc3,j,k,QPRES)    = 3.0e0_rt*q(idx_int1,j,k,QPRES) +10.0e0_rt*q(i,j,k,QPRES) - 18.0e0_rt*q(idx_gc1,j,k,QPRES) &
            + 6.0e0_rt*q(idx_gc2,j,k,QPRES) - 12.0e0_rt*delta*dp*isign

       q(idx_gc4,j,k,QU)    = -TWO*q(idx_int1,j,k,QU) - 13.0e0_rt*q(i,j,k,QU) + 24.0e0_rt*q(idx_gc1,j,k,QU) - 12.0e0_rt*q(idx_gc2,j,k,QU)  &
            + 4.0e0_rt*q(idx_gc3,j,k,QU) + 12.0e0_rt*delta*du*isign
       q(idx_gc4,j,k,QV)    = -TWO*q(idx_int1,j,k,QV) - 13.0e0_rt*q(i,j,k,QV) + 24.0e0_rt*q(idx_gc1,j,k,QV) - 12.0e0_rt*q(idx_gc2,j,k,QV) &
            + 4.0e0_rt*q(idx_gc3,j,k,QV) + 12.0e0_rt*delta*dv*isign
       q(idx_gc4,j,k,QW)    = -TWO*q(idx_int1,j,k,QW) - 13.0e0_rt*q(i,j,k,QW) + 24.0e0_rt*q(idx_gc1,j,k,QW) - 12.0e0_rt*q(idx_gc2,j,k,QW) &
            + 4.0e0_rt*q(idx_gc3,j,k,QW) + 12.0e0_rt*delta*dw*isign
       q(idx_gc4,j,k,QRHO)  = -TWO*q(idx_int1,j,k,QRHO) - 13.0e0_rt*q(i,j,k,QRHO) + 24.0e0_rt*q(idx_gc1,j,k,QRHO) - 12.0e0_rt*q(idx_gc2,j,k,QRHO) &
            + 4.0e0_rt*q(idx_gc3,j,k,QRHO) + 12.0e0_rt*delta*drho*isign
       q(idx_gc4,j,k,QPRES) = -TWO*q(idx_int1,j,k,QPRES) - 13.0e0_rt*q(i,j,k,QPRES) + 24.0e0_rt*q(idx_gc1,j,k,QPRES) - 12.0e0_rt*q(idx_gc2,j,k,QPRES) &
            + 4.0e0_rt*q(idx_gc3,j,k,QPRES) + 12.0e0_rt*delta*dp*isign
       ...
       do hop=idx_start,idx_end,-isign
          ...
          eos_state % xn = q(hop,j,k,QFS:QFS+nspec-1)
          ...
          uin(hop,j,k,UEDEN) = eos_state % rho  &
               * (eos_state % e + HALF * (uin(hop,j,k,UMX)**2 + uin(hop,j,k,UMY)**2 + uin(hop,j,k,UMZ)**2))
          uin(hop,j,k,UTEMP) = eos_state % T
          do n=1, nspec
             uin(hop,j,k,UFS+n-1) = eos_state % rho  *  eos_state % xn(n)
          end do
    ...
    elseif (idir == 2) then
       ...
       ! Update ghost cells
       ! 2nd order
       q(i,idx_gc1,k,QU)    = q(i,idx_int1,k,QU) - TWO*delta*du*isign
       q(i,idx_gc1,k,QV)    = q(i,idx_int1,k,QV) - TWO*delta*dv*isign
       q(i,idx_gc1,k,QW)    = q(i,idx_int1,k,QW) - TWO*delta*dw*isign
       q(i,idx_gc1,k,QRHO)  = q(i,idx_int1,k,QRHO)  - TWO*delta*drho*isign
       q(i,idx_gc1,k,QPRES) = q(i,idx_int1,k,QPRES) - TWO*delta*dp*isign

       !----------------
       q(i,idx_gc2,k,QU)    = -TWO*q(i,idx_int1,k,QU) - 3.0e0_rt*q(i,j,k,QU) + 6.0e0_rt*q(i,idx_gc1,k,QU) + 6.0e0_rt*delta*du*isign
       q(i,idx_gc2,k,QV)    = -TWO*q(i,idx_int1,k,QV) - 3.0e0_rt*q(i,j,k,QV) + 6.0e0_rt*q(i,idx_gc1,k,QV) + 6.0e0_rt*delta*dv*isign
       q(i,idx_gc2,k,QW)    = -TWO*q(i,idx_int1,k,QW) - 3.0e0_rt*q(i,j,k,QW) + 6.0e0_rt*q(i,idx_gc1,k,QW) + 6.0e0_rt*delta*dw*isign
       q(i,idx_gc2,k,QRHO)  = -TWO*q(i,idx_int1,k,QRHO) - 3.0e0_rt*q(i,j,k,QRHO) + 6.0e0_rt*q(i,idx_gc1,k,QRHO) + 6.0e0_rt*delta*drho*isign
       q(i,idx_gc2,k,QPRES) = -TWO*q(i,idx_int1,k,QPRES) - 3.0e0_rt*q(i,j,k,QPRES) + 6.0e0_rt*q(i,idx_gc1,k,QPRES) + 6.0e0_rt*delta*dp*isign

       q(i,idx_gc3,k,QU)    = 3.0e0_rt*q(i,idx_int1,k,QU) +10.0e0_rt*q(i,j,k,QU) - 18.0e0_rt*q(i,idx_gc1,k,QU) &
            + 6.0e0_rt*q(i,idx_gc2,k,QU) - 12.0e0_rt*delta*du*isign
       q(i,idx_gc3,k,QV)    = 3.0e0_rt*q(i,idx_int1,k,QV) +10.0e0_rt*q(i,j,k,QV) - 18.0e0_rt*q(i,idx_gc1,k,QV) &
            + 6.0e0_rt*q(i,idx_gc2,k,QV) - 12.0e0_rt*delta*dv*isign
       q(i,idx_gc3,k,QW)    = 3.0e0_rt*q(i,idx_int1,k,QW) +10.0e0_rt*q(i,j,k,QW) - 18.0e0_rt*q(i,idx_gc1,k,QW) &
            + 6.0e0_rt*q(i,idx_gc2,k,QW) - 12.0e0_rt*delta*dw*isign
       q(i,idx_gc3,k,QRHO)  = 3.0e0_rt*q(i,idx_int1,k,QRHO) +10.0e0_rt*q(i,j,k,QRHO) - 18.0e0_rt*q(i,idx_gc1,k,QRHO) &
            + 6.0e0_rt*q(i,idx_gc2,k,QRHO) - 12.0e0_rt*delta*drho*isign
       q(i,idx_gc3,k,QPRES) = 3.0e0_rt*q(i,idx_int1,k,QPRES) +10.0e0_rt*q(i,j,k,QPRES) - 18.0e0_rt*q(i,idx_gc1,k,QPRES) &
            + 6.0e0_rt*q(i,idx_gc2,k,QPRES) - 12.0e0_rt*delta*dp*isign

       q(i,idx_gc4,k,QU)    = -TWO*q(i,idx_int1,k,QU) - 13.0e0_rt*q(i,j,k,QU) + 24.0e0_rt*q(i,idx_gc1,k,QU) - 12.0e0_rt*q(i,idx_gc2,k,QU)  &
            + 4.0e0_rt*q(i,idx_gc3,k,QU) + 12.0e0_rt*delta*du*isign
       q(i,idx_gc4,k,QV)    = -TWO*q(i,idx_int1,k,QV) - 13.0e0_rt*q(i,j,k,QV) + 24.0e0_rt*q(i,idx_gc1,k,QV) - 12.0e0_rt*q(i,idx_gc2,k,QV) &
            + 4.0e0_rt*q(i,idx_gc3,k,QV) + 12.0e0_rt*delta*dv*isign
       q(i,idx_gc4,k,QW)    = -TWO*q(i,idx_int1,k,QW) - 13.0e0_rt*q(i,j,k,QW) + 24.0e0_rt*q(i,idx_gc1,k,QW) - 12.0e0_rt*q(i,idx_gc2,k,QW) &
            + 4.0e0_rt*q(i,idx_gc3,k,QW) + 12.0e0_rt*delta*dw*isign
       q(i,idx_gc4,k,QRHO)  = -TWO*q(i,idx_int1,k,QRHO) - 13.0e0_rt*q(i,j,k,QRHO) + 24.0e0_rt*q(i,idx_gc1,k,QRHO) - 12.0e0_rt*q(i,idx_gc2,k,QRHO) &
            + 4.0e0_rt*q(i,idx_gc3,k,QRHO) + 12.0e0_rt*delta*drho*isign
       q(i,idx_gc4,k,QPRES) = -TWO*q(i,idx_int1,k,QPRES) - 13.0e0_rt*q(i,j,k,QPRES) + 24.0e0_rt*q(i,idx_gc1,k,QPRES) - 12.0e0_rt*q(i,idx_gc2,k,QPRES) &
            + 4.0e0_rt*q(i,idx_gc3,k,QPRES) + 12.0e0_rt*delta*dp*isign
       ...
       do hop=idx_start,idx_end,-isign
          ...
          eos_state % xn = q(i,hop,k,QFS:QFS+nspec-1)
          ...
          uin(i,hop,k,UEDEN) = eos_state % rho  &
               * (eos_state % e + HALF * (uin(i,hop,k,UMX)**2 + uin(i,hop,k,UMY)**2 + uin(i,hop,k,UMZ)**2))
          uin(i,hop,k,UTEMP) = eos_state % T
          do n=1, nspec
             uin(i,hop,k,UFS+n-1) = eos_state % rho  *  eos_state % xn(n)
          end do
    ...
    elseif (idir == 3) then
       ...
       ! Update ghost cells
       ! 2nd order
       q(i,j,idx_gc1,QU)    = q(i,j,idx_int1,QU) - TWO*delta*du*isign
       q(i,j,idx_gc1,QV)    = q(i,j,idx_int1,QV) - TWO*delta*dv*isign
       q(i,j,idx_gc1,QW)    = q(i,j,idx_int1,QW) - TWO*delta*dw*isign
       q(i,j,idx_gc1,QRHO)  = q(i,j,idx_int1,QRHO)  - TWO*delta*drho*isign
       q(i,j,idx_gc1,QPRES) = q(i,j,idx_int1,QPRES) - TWO*delta*dp*isign

       !----------------
       q(i,j,idx_gc2,QU)    = -TWO*q(i,j,idx_int1,QU) - 3.0e0_rt*q(i,j,k,QU) + 6.0e0_rt*q(i,j,idx_gc1,QU) + 6.0e0_rt*delta*du*isign
       q(i,j,idx_gc2,QV)    = -TWO*q(i,j,idx_int1,QV) - 3.0e0_rt*q(i,j,k,QV) + 6.0e0_rt*q(i,j,idx_gc1,QV) + 6.0e0_rt*delta*dv*isign
       q(i,j,idx_gc2,QW)    = -TWO*q(i,j,idx_int1,QW) - 3.0e0_rt*q(i,j,k,QW) + 6.0e0_rt*q(i,j,idx_gc1,QW) + 6.0e0_rt*delta*dw*isign
       q(i,j,idx_gc2,QRHO)  = -TWO*q(i,j,idx_int1,QRHO) - 3.0e0_rt*q(i,j,k,QRHO) + 6.0e0_rt*q(i,j,idx_gc1,QRHO) + 6.0e0_rt*delta*drho*isign
       q(i,j,idx_gc2,QPRES) = -TWO*q(i,j,idx_int1,QPRES) - 3.0e0_rt*q(i,j,k,QPRES) + 6.0e0_rt*q(i,j,idx_gc1,QPRES) + 6.0e0_rt*delta*dp*isign

       q(i,j,idx_gc3,QU)    = 3.0e0_rt*q(i,j,idx_int1,QU) +10.0e0_rt*q(i,j,k,QU) - 18.0e0_rt*q(i,j,idx_gc1,QU) &
            + 6.0e0_rt*q(i,j,idx_gc2,QU) - 12.0e0_rt*delta*du*isign
       q(i,j,idx_gc3,QV)    = 3.0e0_rt*q(i,j,idx_int1,QV) +10.0e0_rt*q(i,j,k,QV) - 18.0e0_rt*q(i,j,idx_gc1,QV) &
            + 6.0e0_rt*q(i,j,idx_gc2,QV) - 12.0e0_rt*delta*dv*isign
       q(i,j,idx_gc3,QW)    = 3.0e0_rt*q(i,j,idx_int1,QW) +10.0e0_rt*q(i,j,k,QW) - 18.0e0_rt*q(i,j,idx_gc1,QW) &
            + 6.0e0_rt*q(i,j,idx_gc2,QW) - 12.0e0_rt*delta*dw*isign
       q(i,j,idx_gc3,QRHO)  = 3.0e0_rt*q(i,j,idx_int1,QRHO) +10.0e0_rt*q(i,j,k,QRHO) - 18.0e0_rt*q(i,j,idx_gc1,QRHO) &
            + 6.0e0_rt*q(i,j,idx_gc2,QRHO) - 12.0e0_rt*delta*drho*isign
       q(i,j,idx_gc3,QPRES) = 3.0e0_rt*q(i,j,idx_int1,QPRES) +10.0e0_rt*q(i,j,k,QPRES) - 18.0e0_rt*q(i,j,idx_gc1,QPRES) &
            + 6.0e0_rt*q(i,j,idx_gc2,QPRES) - 12.0e0_rt*delta*dp*isign

       q(i,j,idx_gc4,QU)    = -TWO*q(i,j,idx_int1,QU) - 13.0e0_rt*q(i,j,k,QU) + 24.0e0_rt*q(i,j,idx_gc1,QU) - 12.0e0_rt*q(i,j,idx_gc2,QU)  &
            + 4.0e0_rt*q(i,j,idx_gc3,QU) + 12.0e0_rt*delta*du*isign
       q(i,j,idx_gc4,QV)    = -TWO*q(i,j,idx_int1,QV) - 13.0e0_rt*q(i,j,k,QV) + 24.0e0_rt*q(i,j,idx_gc1,QV) - 12.0e0_rt*q(i,j,idx_gc2,QV) &
            + 4.0e0_rt*q(i,j,idx_gc3,QV) + 12.0e0_rt*delta*dv*isign
       q(i,j,idx_gc4,QW)    = -TWO*q(i,j,idx_int1,QW) - 13.0e0_rt*q(i,j,k,QW) + 24.0e0_rt*q(i,j,idx_gc1,QW) - 12.0e0_rt*q(i,j,idx_gc2,QW) &
            + 4.0e0_rt*q(i,j,idx_gc3,QW) + 12.0e0_rt*delta*dw*isign
       q(i,j,idx_gc4,QRHO)  = -TWO*q(i,j,idx_int1,QRHO) - 13.0e0_rt*q(i,j,k,QRHO) + 24.0e0_rt*q(i,j,idx_gc1,QRHO) - 12.0e0_rt*q(i,j,idx_gc2,QRHO) &
            + 4.0e0_rt*q(i,j,idx_gc3,QRHO) + 12.0e0_rt*delta*drho*isign
       q(i,j,idx_gc4,QPRES) = -TWO*q(i,j,idx_int1,QPRES) - 13.0e0_rt*q(i,j,k,QPRES) + 24.0e0_rt*q(i,j,idx_gc1,QPRES) - 12.0e0_rt*q(i,j,idx_gc2,QPRES) &
            + 4.0e0_rt*q(i,j,idx_gc3,QPRES) + 12.0e0_rt*delta*dp*isign
       ...
       do hop=idx_start,idx_end,-isign
          ...
          eos_state % xn = q(i,j,hop,QFS:QFS+nspec-1)
          ...
          uin(i,j,hop,UEDEN) = eos_state % rho  &
               * (eos_state % e + HALF * (uin(i,j,hop,UMX)**2 + uin(i,j,hop,UMY)**2 + uin(i,j,hop,UMZ)**2))
          uin(i,j,hop,UTEMP) = eos_state % T
          do n=1, nspec
             uin(i,j,hop,UFS+n-1) = eos_state % rho  *  eos_state % xn(n)
          end do
  ...
  subroutine compute_waves(i, j, k, idir, isign, &
    ...
    use meth_params_module, only : NQ, QPRES, QU, QV, QW, QRHO, NQAUX, QC, QGAMC, QTEMP
    ...
    if ((idir == 1) .or. (idir == 2) .or. (idir == 3)) then
       ...
       call amrex_abort("Problem of idir in impose_NSCBC_3d:compute_waves")
    end if

    if ((isign == 1) .or. (isign == -1)) then
       continue
    else
       call amrex_abort("Problem of isign in impose_NSCBC_3d:compute_waves")
    end if

    mach_local = dsqrt(q(i,j,k,QU)**TWO + q(i,j,k,QV)**TWO + q(i,j,k,QW)**TWO)/qaux(i,j,k,QC)
    ...
    if (idir == 1) then
       ...
       L2 =  q(i,j,k,QU) * ( ((qaux(i,j,k,QC)**TWO)*drho) - dp)
       L3 =  q(i,j,k,QU) * dv
       L4 =  q(i,j,k,QU) * dw
    ...
    elseif (idir == 2) then
       ...
       L2 =  q(i,j,k,QV) * du
       L3 =  q(i,j,k,QV) * ( ((qaux(i,j,k,QC)**TWO)*drho) - dp)
       L4 =  q(i,j,k,QV) * dw
    ...
    elseif (idir == 3) then
       ...
       L2 =  q(i,j,k,QW) * du
       L3 =  q(i,j,k,QW) * dv
       L4 =  q(i,j,k,QW) * ( ((qaux(i,j,k,QC)**TWO)*drho) - dp)
    ...
    if (bc_type == Inflow) then
       ...
       if (idir == 1) then
          if (isign == 1) then
             L5 = relax_U * ((q(i,j,k,QRHO)*qaux(i,j,k,QC)**TWO)*(ONE-mach_local*mach_local)/probhi(idir)) * &
                  (q(i,j,k,QU) - TARGET_VX)  - ((ONE - beta)*T5)
          elseif (isign == -1) then
             L1 = relax_U * ((q(i,j,k,QRHO)*qaux(i,j,k,QC)**TWO)*(ONE-mach_local*mach_local)/probhi(idir)) * &
                  (q(i,j,k,QU) - TARGET_VX) - ((ONE - beta)*T1)
          endif

          L2 = relax_T * (q(i,j,k,QRHO)*qaux(i,j,k,QC)/probhi(idir)) &
               * (q(i,j,k,QTEMP) - TARGET_TEMPERATURE) - ((ONE - beta)*T2)
       ...
       elseif (idir == 2) then
          ...
          if (isign == 1) then
             L5 = relax_V * ((q(i,j,k,QRHO)*qaux(i,j,k,QC)**TWO)*(ONE-mach_local*mach_local)/probhi(idir)) * &
                  (q(i,j,k,QV) - TARGET_VY) - ((ONE - beta)*T5)
          elseif (isign == -1) then
             L1 = relax_V * ((q(i,j,k,QRHO)*qaux(i,j,k,QC)**TWO)*(ONE-mach_local*mach_local)/probhi(idir)) * &
                  (q(i,j,k,QV) - TARGET_VY)  -  ((ONE - beta)*T1)
          endif

          L2 = relax_U * (qaux(i,j,k,QC)/probhi(idir)) * (q(i,j,k,QU) - TARGET_VX)  &
               - ((ONE - beta)*T2)
          L3 = relax_T * (q(i,j,k,QRHO)*qaux(i,j,k,QC)/probhi(idir)) &
               * (q(i,j,k,QTEMP) - TARGET_TEMPERATURE) - ((ONE - beta)*T3)
          L4 = relax_W * (qaux(i,j,k,QC)/probhi(idir)) * (q(i,j,k,QW) - TARGET_VZ) &
               - ((ONE - beta)*T4)
       ...
       elseif (idir == 3) then
          ...
          if (isign == 1) then
             L5 = relax_W * ((q(i,j,k,QRHO)*qaux(i,j,k,QC)**TWO)*(ONE-mach_local*mach_local)/probhi(idir)) * &
                  (q(i,j,k,QW) - TARGET_VZ) - ((ONE - beta)*T5)
          elseif (isign == -1) then
             L1 = relax_W * ((q(i,j,k,QRHO)*qaux(i,j,k,QC)**TWO)*(ONE-mach_local*mach_local)/probhi(idir)) * &
                  (q(i,j,k,QW) - TARGET_VZ)  -  ((ONE - beta)*T1)
          endif
          ...
          L3 = relax_W * (qaux(i,j,k,QC)/probhi(idir)) * (q(i,j,k,QV) - TARGET_VY) &
               - ((ONE - beta)*T3)
          L4 = relax_T * (q(i,j,k,QRHO)*qaux(i,j,k,QC)/probhi(idir)) &
               * (q(i,j,k,QTEMP) - TARGET_TEMPERATURE) - ((ONE - beta)*T4)

       else
          call amrex_error("Error:: Wait, is this the fourth dimension?")
       endif
    ...
    elseif (bc_type == Outflow) then
       ...
       Kout = sigma_out*(ONE - (mach_local**TWO))*(qaux(i,j,k,QC)/probhi(idir))

       if (isign == 1) then
          L5 = (Kout*(q(i,j,k,QPRES) - TARGET_PRESSURE)) - ((ONE - beta)*T5)
       elseif (isign == -1) then
          L1 = (Kout*(q(i,j,k,QPRES) - TARGET_PRESSURE)) - ((ONE - beta)*T1)
       endif

    else
       call amrex_error("Error:: This BC is not yet implemented for x dir in characteristic form")
    endif
