...
subroutine IO_writeIntegralQuantities (isFirst, simTime)
  ...
  use Grid_interface, ONLY : Grid_getListOfBlocks, &
    ...
    Grid_releaseBlkPtr
  use Burn_data, only : c_l, N_A, m_p, m_n, m_e, bn_neutLoss, bn_neutLossThisProcStep, bn_thermalReact, bn_autoDDT, pbIgnRho
...
!=============================================================================!
! find minimum flame density
!  and
! check for conditions necessary to activate auto DDT
!  currently auto detonation is triggered by the flame going below a density
!  threshold (approximately pbIgnRho), so we here we check to see if the
!  minimum flame density is close to that threshold and if it is turn on
!  burning.  This will also enable the actual checks for local detonation
!  conditions, which are in the burning module.
!=============================================================================!

  ! if we are waiting to autoddt, then all processors need to check threshold, so
  !  we need an allreduce.  Otherwise just a reduce to print the minflame value
  if (bn_autoDDT .and. (.not. bn_thermalReact)) then
    call MPI_AllReduce (lminflamdens, gminflamdens, 1, FLASH_REAL, MPI_MIN, io_globalComm, error)
    if ( gminflamdens < 1.1e0*pbIgnRho ) then
      call Logfile_stamp('autodetonations activated', '[IO_writeIntegralQuantities]')
      bn_thermalReact = .true.
    endif
  else
    call MPI_Reduce (lminflamdens, gminflamdens, 1, FLASH_REAL, MPI_MIN, MASTER_PE, io_globalComm, error)
  endif
